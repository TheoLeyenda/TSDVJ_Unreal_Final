...
void ABatteryMan::BeginPlay()
{
	...
	GetCapsuleComponent()->OnComponentBeginOverlap.AddDynamic(this, &ABatteryMan::OnComponentBeginOverlap);
...
void ABatteryMan::Tick(float DeltaTime)
{
	...
	if(Power <= 0)
	{
		...
		if(!bDead)
		{
			...
			//timer para resetear nivel
			float timerRestartLevel = 3.0f;
			FTimerHandle UnsedHandle;
			GetWorldTimerManager().SetTimer(UnsedHandle, this, &ABatteryMan::RestartGame, timerRestartLevel, false);
			//-----------------------//
		}
	}
}
...
void ABatteryMan::OnComponentBeginOverlap(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromeSweep, const FHitResult &SweepResult)
{
